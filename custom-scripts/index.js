...
function createIstanbulParameter(option, command) {
		...
		if (option) {
				commandLine = option
						.split(',')
						.map(commandPath => ' -' + command + ' ' + commandPath)
						.join(' ') + ' ';
		}
...
if (options.test) {
		...
		const subprocess = spawn(mocha + ' --reporter spec ' + options.test.join(' '), {
				stdio: 'inherit',
				shell: true,
				cwd
		});
...
if (options.cover) {
		...
		const subprocess = spawn(istanbul + ' cover ' + createIstanbulParameter(options.exclude, 'x') + createIstanbulParameter(options.include, 'i') + mocha + ' -- -R spec test/unit/**/*.js', {
				stdio: 'inherit',
				shell: true,
				cwd
		});
...
if (options.compile) {
		...
		console.log(chalk.green('Using package.json'));
...
if (options.lint) {
		if (options.lint === 'js') {
				const subprocess = spawn(path.resolve(cwd, './node_modules/.bin/eslint') + ' .', {
						stdio: 'inherit',
						shell: true,
						cwd
				});
		...
		if (options.lint === 'css') {
				const subprocess = spawn(path.resolve(cwd, './node_modules/.bin/stylelint') + ' --syntax scss "**/*.scss"', {
						stdio: 'inherit',
						shell: true,
						cwd
				});
...
if (options.watch) {
		...
		const scssWatcher = chokidar.watch(cartridgesPath + '/**/*.scss', {
				persistent: true,
				ignoreInitial: true,
		...
		const clientJSWatcher = chokidar.watch(cartridgesPath + '/**/client/**/*.js', {
				persistent: true,
				ignoreInitial: true,
				followSymlinks: false,
				awaitWriteFinish: {
		...
		if (!options.onlycompile) {
				const watcher = chokidar.watch(cartridgesPath, {
						ignored: [
								'**/cartridge/js/**', '**/cartridge/client/**', '**/*.scss'
						],
						persistent: true,
						ignoreInitial: true,
						followSymlinks: false,

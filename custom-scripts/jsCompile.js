...
module.exports = (packageFile, pwd, callback) => {
		...
		if (modules) {
				...
				const currentJsDir = path.join(cwd, 'cartridge/', 'client/default/js');
				console.log('currentJsDir', currentJsDir);
				shell.mkdir('-p', TEMP_DIR);
				shell.cp('-r', currentJsDir, TEMP_JS_DIR);
				Object
						.keys(modules)
						.forEach(key => {
								const currentPath = path.join(cwd, modules[key], 'cartridge/client/default/js');
		...
		const subprocess = spawn(webpack, {
				stdio: 'inherit',
				shell: true,
				cwd
		});
